...
describe('GET /pflanzen/:id', () => {
    ...
    each(idVorhanden).test('Pflanze zu vorhandener ID %s', async (id) => {
        // given
    ...
    each(idNichtVorhanden).test(
        'Kein Pflanze zu nicht-vorhandener ID %s',
        async (id) => {
            // given
            const uri = `${buecherUri}/${id}`;
    ...
    each(idVorhandenETag).test(
        'Pflanze zu vorhandener ID %s mit ETag %s',
        async (id, etag) => {
            // given
            const uri = `${buecherUri}/${id}`;
